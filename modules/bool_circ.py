...
class bool_circ(open_digraph):
	...
	def adder(cls, a, b, carry):
		"""
		:param a: registre de taille 2**n
		:param b: registre de taille 2**n
		...
		for i in ids:
			...
			if node_i.get_label() != "1":
				...
				for keys in list(node_i.get_children_ids()):
					...
					for removeParent in list(self.get_node_by_id(keys).get_parent_ids()):
						self.remove_parallel_edges((removeParent, keys))
	...
	def evaluate(self):
		while True:
			...
			condition = [all(x in self.get_output_ids() for x in nodeI.get_children_ids()) for nodeI in inputNodes]

			if all(condition):
				break

			for i, currentNode in enumerate(inputNodes):
				if not condition[i]:
		...
		for n in inputNodes:
			if n.get_label() != "1" and n.get_label() != "0":
				self.element_Neutre(n.get_id())
	...
	def evaluatePlusPlus(self):
		while True:
			allNode = [n for n in self.get_nodes() if n.get_children_ids()]
			...
			condition = [all(x in self.get_output_ids() for x in n.get_children_ids()) for n in inputNodes]
		...
		for n in self.get_nodes():
			if not n.get_children_ids() and not n.get_parent_ids():
				n.set_label("")
